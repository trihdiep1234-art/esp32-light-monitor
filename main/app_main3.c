...
// Tốc độ I2C dùng khi ĐỌC BH1750 (Hz). Đang hạ xuống 50kHz (thấp hơn
// chuẩn Fast-mode 400kHz) để tăng độ ổn định khi đọc, đánh đổi lại là
// mỗi lần đọc tốn nhiều thời gian hơn trên bus I2C.
...
void app_main(void)
{
    ...
    // Khởi tạo thông số mô tả mặc định cho BH1750
    ...
    // THAY ĐỔI QUAN TRỌNG: Ghi đè tốc độ I2C xuống 50kHz trước khi setup chip
    ...
    // Khởi tạo biến để lưu timing gửi lên ThingsBoard
    double dt_sec = 1.0; 
    double f_actual = 1.0; 
    ...
    while (1)
    {
        ...
        if (last_sample_time != 0) {
            ...
            error_pct = (dt_sec - 1.0) * 100.0;

            ESP_LOGI(TAG, "[TIMING] dt=%.4fs | f_actual=%.4fHz | error=%.2f%%",
        ...
        snprintf(payload, sizeof(payload), 
                 "{\"lux\":%d,\"temt\":%.2f,\"dt\":%.4f,\"f_actual\":%.4f,\"error_pct\":%.2f}", 
                 bh_lux, (float)temt_lux, dt_sec, f_actual, error_pct);
