#include <stdint.h> // Thêm thư viện định nghĩa kiểu uint32_t chuẩn
#include "sds011.h"
#include "assert.h"
...
void sds011_begin(const uart_port_t uart_num,
  ...
  // [SỬA ĐỔI QUAN TRỌNG]: Đưa cài đặt Driver lên trước để cấp phát tài nguyên hệ thống
  ESP_ERROR_CHECK(uart_driver_install(
      sds011_uart_num, SDS011_UART_RX_BUFFER_SIZE, SDS011_UART_TX_BUFFER_SIZE,
      SDS011_UART_EVENT_QUEUE_SIZE, NULL, 0));
ESP_LOGI("CHECK_CODE", "--- TOI DA DAO THU TU HAM THANH CONG ---");
  // Cấu hình các thông số Baudrate, Data bits...
  ESP_ERROR_CHECK(uart_param_config(sds011_uart_num, &uart_config));

  // [SỬA ĐỔI QUAN TRỌNG]: Map chân GPIO vật lý ở BƯỚC CUỐI CÙNG để không bị ghi đè
...
void sds011_tx_task(void* pvParameters) {
  ...
  // [SỬA ĐỔI]: Thay đổi uint thành uint32_t để tương thích với compiler hiện tại
  uint32_t packet_send_remaining = 0;
...
void sds011_rx_task(void* pvParameters) {
  ...
  for (;;) {
    // Bước 1: Tìm byte bắt đầu (Header = 0xAA)
    uint8_t head_byte = 0;
    int read_head = uart_read_bytes(sds011_uart_num, &head_byte, 1, pdMS_TO_TICKS(1000));
    
    if (read_head <= 0) {
      ESP_LOGW(TAG, "Không nhận được dữ liệu từ SDS011 (Timeout 1s)");
      continue;
    }
    
    if (head_byte != SDS011_PACKET_HEAD) {
      // Nếu không phải 0xAA, bỏ qua và tìm tiếp byte sau
      continue; 
    }
    
    // Đã tìm thấy Header 0xAA, nạp vào vị trí đầu tiên của struct
    packet_recv_buf[0] = head_byte;
    
    // Bước 2: Đọc nốt 9 bytes còn lại của gói tin
    uint32_t remain = sizeof(packet_recv) - 1; // 9 bytes
    int read_remain = uart_read_bytes(sds011_uart_num, &packet_recv_buf[1], remain, pdMS_TO_TICKS(200));
    
    if (read_remain == remain) {
      // Đã nhận đủ 10 bytes, tiến hành kiểm tra tính toàn vẹn
      if (sds011_rx_check_frame(&packet_recv) && sds011_rx_check_checksum(&packet_recv)) {
        
        ESP_LOGI(TAG, "Đã nhận gói tin hợp lệ từ SDS011!");
        
        switch (packet_recv.command) {
          case SDS011_RX_CMD_SENSOR_DATA:
#if SDS011_RX_DATA_QUEUE_SIZE == 1
            xQueueOverwrite(sds011_rx_data_queue, (const void*)&packet_recv);
#else
            xQueueSendToBack(sds011_rx_data_queue, (const void *)&packet_recv, 0);
#endif
            break;
          default:
#if SDS011_RX_CMD_QUEUE_SIZE == 1
            xQueueOverwrite(sds011_rx_cmd_queue, (const void*)&packet_recv);
#else
            xQueueSendToBack(sds011_rx_cmd_queue, (const void *)&packet_recv, 0);
#endif
            break;
        }
      } else {
        ESP_LOGE(TAG, "Gói tin sai Khung hoặc sai Checksum!");
      }
    } else {
      ESP_LOGW(TAG, "Bị đứt đoạn, không đọc đủ 9 bytes còn lại (recv=%d)", read_remain);
    }
...
bool sds011_rx_check_checksum(const struct sds011_rx_packet* packet) {
  // [SỬA ĐỔI]: Thay đổi uint thành uint32_t
  uint32_t checksum = 0;
  const uint8_t* bytes =
